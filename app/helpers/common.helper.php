...
function general_data($name, $field = null)
{
  ...
  if (isset($GLOBALS["General_Data"][$name])) {
    ...
    $settings = Controller::model("Site", $name);
    $GLOBALS["General_Data"][$name] = $settings;
  }


  if (is_string($field)) {
    return htmlchars($settings->get("settings." . $field));
  }
...
{
  $idname = $_SERVER["SERVER_NAME"];
  return general_data($idname, $field);
}
...
/**
 * Filter list ID
 * @param $data is list ID without filter
 * @return array list is only ID as number
 */
function filterListID($data)
{
  $return = [];
  if (!is_array($data)) {
    return $return;
  ...
  foreach ($data as $item) {
    if (is_numeric($item) && strlen($item) < 16) {
      $return[] = $item;
    }
  }
...
function convertSeedTypeToLog($seed_type)
{
  $dump = explode('-', $seed_type);
  return strtoupper($dump[1]);
}
